...
namespace Base_NEAT
{
    class Genome
    {
        ...
        public static Genome Crossover(Genome parent1, Genome pParent2, Random pRandom)
        {
            ...
            AddNodesToChild(child, leastFit, mostFit, pRandom, sameFitness);
        ...
        private static void AddNodesToChild(Genome pChild, Genome pLeastFit, Genome pMostFit, Random pRandom, bool pSameFitness)
        {
        ...
        public void Mutate(Random pRandom)
        {
            ...
            if (doMutate < NEATConfig.MUTATION_RATE)
            {
                ...
                float doAllWeightMutate = (float)pRandom.NextDouble();
                float doAddConnection = (float)pRandom.NextDouble();
                float doAddNode = (float)pRandom.NextDouble();
                float doRemoveConnection = (float)pRandom.NextDouble();
                float doRemoveNode = (float)pRandom.NextDouble();
                if (doWeightMutate < NEATConfig.SINGLE_WEIGHT_MUTATE_RATE)
                {
                    SingleWeightMutation(pRandom);
                }
                if (doAllWeightMutate < NEATConfig.ALL_WEIGHT_MUTATE_RATE)
                {
                    AllWeightMutation(pRandom);
                }
        ...
        private void AllWeightMutation(Random pRandom)
        {
        ...
        private void SingleWeightMutation(Random pRandom)
        {
            int randomConnection = pRandom.Next(0, _connections.Count);
            _connections[randomConnection].Weight = (float)pRandom.NextDouble();
        }
        ...
        public static float Compare(Genome pParent1, Genome pParent2)
        {
            ...
            List<ConnectionGene> parent1Conn = pParent1._connections.OrderByDescending(entry => entry.InnovationNumber).ToList();
            List<ConnectionGene> parent2Conn = pParent2._connections.OrderByDescending(entry => entry.InnovationNumber).ToList();
            ...
            return totalExcess + totalDisjoint + totalMatching;
        }
    }
}
